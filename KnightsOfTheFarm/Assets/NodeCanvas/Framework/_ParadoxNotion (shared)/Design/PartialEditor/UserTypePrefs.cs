...
namespace ParadoxNotion.Design{
    ...
    public static class UserTypePrefs {
		...
		/// 'includeFunctionalTypes' will NOT exclude class types that only have static method and property members
		/// which are typicaly not used for variables. Of course thats a big assumption here
		public static List<Type> GetPreferedTypesList(Type baseType, bool includeFunctionalTypes = false){
			...
			if (!includeFunctionalTypes){
				foreach (var t in types.ToArray()){
					if (!t.IsEnum && t.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly).Length == 0){
						types.Remove(t);
					}
				}
			}
